...
class MultiRunTests: XCTestCase {
    ...
    /// Tests that creating a `MultiRun` with `Run`s that do not contain all the `Pair Card`s throws an
    /// `invalidRun Error`.
    func test_init_withRunsNotContainingAllPairCards_throwsInvalidMinError()
        throws {
