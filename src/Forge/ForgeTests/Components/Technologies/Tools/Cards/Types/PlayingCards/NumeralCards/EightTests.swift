...
class EightTests: XCTestCase {
    ...
    /// Tests that creating an`Eight` with a`null PlayingCard Suit` throws an `invalidSuit`
    /// `Error`.
    func test_init_withNullSuit_throwsInvalidSuit() throws {
        ...
        // When
        XCTAssertThrowsError(try Eight(of: .null)) { error in
    ...
    /// Tests that an `Eight` `Rank`s another `Eight`.
    func test_eight_ranksEight_true() throws {
        ...
        // When/Then
        XCTAssert(eightOfHearts.ranks(eightOfSpades))
    }
    
    /// Tests that an `Eight` does not `Rank` a `Nine`.
    func test_eight_ranksNine_false() throws {
        ...
        // When/Then
        XCTAssertFalse(eight.ranks(nine))
    }
    ...
    /// Tests that an `Eight` follows a `Seven`.
    func test_eight_followsASeven_true() throws {
